...
if ( ! empty( $full_width_css ) ) {
	$css_classes[] = 'porto_stretch_css';
	$css_classes[] = $full_width_css;
}
...
// particles effect
if ( ! empty( $atts['particles_effect'] ) && defined( 'PORTO_FUNC_VERSION' ) && ! empty( $atts['particles_img'] ) && is_numeric( $atts['particles_img'] ) ) {
...
// scroll parallax
if ( ! empty( $atts['scroll_parallax'] ) && defined( 'PORTO_FUNC_VERSION' ) ) {
...
if ( ! empty( $atts['scroll_inviewport'] ) ) {
	$extra_options = array();
	if ( ! empty( $atts['scroll_bg_scale'] ) ) {
		wp_enqueue_script( 'porto-gsap' );
		wp_enqueue_script( 'porto-scroll-trigger' );
		$extra_options['scroll_bg_scale'] = true;
		if ( ! empty( $atts['scale_bg'] ) ) {
			$extra_options['scale_bg'] = $atts['scale_bg'];
		}
		$extra_options['scale_extra_class'] = $atts['scale_extra_class'];
		if ( ! empty( $atts['set_round'] ) ) {
