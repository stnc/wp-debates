...
$mobile_vars      = '';
$has_shop            = class_exists( 'WooCommerce' );
$container_selectors = '';
		...
		if ( isset( $line[2] ) && 'typography' == $line[2] ) {
			...
			foreach ( $typography_vars as $tv => $ab_name ) {
				if ( ! empty( $b[ $option_name ][ $tv ] ) ) {
					...
					if ( 'ff' == $ab_name && false === strpos( $var_val, ',' ) ) {
						$var_val = '"' . $var_val . '"';
					}
					echo '--porto-' . esc_html( $var_name ) . '-' . $ab_name . ':' . sanitize_text_field( $var_val ) . ';';
				} elseif ( $has_shop && 'ls' == $ab_name && in_array( $option_name, $responsive_fonts ) && 'body-font' != $option_name ) {
					echo '--porto-' . esc_html( $var_name ) . '-' . $ab_name . ': normal;';
				}
			}
...
--porto-sticky-logo-mw: <?php echo empty( $porto_settings['logo-width-sticky'] ) ? 'var(--porto-logo-mw)' : ( empty( $porto_settings['change-header-logo'] ) ? (int) $porto_settings['logo-width-sticky'] : (int) $porto_settings['logo-width-sticky'] * 1.25 ) . 'px'; ?>;
}
...
/* responsive */
<?php if ( (int) $porto_settings['container-width'] >= 1360 ) : ?>
	@media (min-width: <?php echo 1140 + (int) $porto_settings['grid-gutter-width']; ?>px) and (max-width: <?php echo (int) $porto_settings['container-width'] + 2 * (int) $porto_settings['grid-gutter-width'] - 1; ?>px) { 
		<?php
		if ( ! empty( $container_selectors ) ) {
			echo esc_html( $container_selectors ) . '{ --porto-container-width: 1140px }';
		}
		?>
	}
	@media (min-width: 992px) and (max-width: <?php echo 1140 + (int) $porto_settings['grid-gutter-width'] - 1; ?>px) {
<?php else : ?>
