...
if ( $builder_id ) {
	...
	$wrap_cls   = '';
	$wrap_attrs = '';
	if ( ! empty( $porto_settings['blog-infinite'] ) &&  ! ( ! empty( $post_options ) && get_metadata( $term->taxonomy, $term->term_id, 'post_infinite', true ) == 'post_infinite' ) ) {
	...
	if ( 'timeline' == $post_layout ) {
		...
		$post_loop_start .= '<div class="blog-posts posts-' . esc_attr( $post_layout ) . ( empty( $porto_settings['post-style'] ) ? '' : ' blog-posts-' . esc_attr( $porto_settings['post-style'] ) ) . '">';
