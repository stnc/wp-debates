...
if ( ! function_exists( 'porto_force_balance_tags' ) ) {
	/**
	 ...
	 */
	function porto_force_balance_tags( $markup ) {
...
if ( ! function_exists( 'porto_get_excerpt' ) ) :
	function porto_get_excerpt( $limit = 45, $more_link = true, $more_style_block = false, $render_block = true ) {
		...
		$content = porto_strip_excerpt_tags( porto_the_content( $content, false, $render_block ) );		
		...
		if ( $content ) {
			$content = wp_kses_post( porto_force_balance_tags( $content ) );
		}
		if ( $more_link ) {
			if ( $more_style_block ) {
...
if ( ! function_exists( 'porto_ajax_posts' ) ) :
	...
	function porto_ajax_posts() {
		...
		// compatibility with yith ajax product filters plugin
		if ( ! empty( $_REQUEST['current_link'] ) && false !== strpos( $_REQUEST['current_link'], 'yith_wcan=' ) ) {
			$query_string = parse_url( sanitize_url( $_REQUEST['current_link'] ), PHP_URL_QUERY );
			if ( $query_string ) {
				...
				if ( ! empty( $get_params ) ) {
					...
					if ( class_exists( 'YITH_WCAN_Query' ) && ! did_action( 'wp' ) ) {
						YITH_WCAN_Query::instance()->start_filtering();
					}
				}
			}
		}
		...
		if ( $is_shortcode ) {
			if ( isset( $_GET['extra'] ) ) {
				$atts = json_decode( sanitize_text_field( wp_unslash( $_GET['extra'] ) ), true );
			} else {
				$atts = array_map( 'sanitize_text_field', $_REQUEST['extra'] );
			}
			...
			if ( ! empty( $_REQUEST['category'] ) ) {
				$atts['cats'] = sanitize_title( $_REQUEST['category'] );
			}
			...
			if ( ! empty( $_REQUEST['category'] ) ) {
				$cats       = sanitize_title( $_REQUEST['category'] );
			...
			if ( $posts->have_posts() ) {
				...
				if ( 'post' != $post_type && isset( $porto_settings[ $post_type . '-cat-sort-pos' ] ) && 'content' === $porto_settings[ $post_type . '-cat-sort-pos' ] ) {
					...
					if ( isset( $porto_settings[ $post_type . '-infinite' ] ) && 'infinite' != $porto_settings[ $post_type . '-infinite' ] && 'load_more' != $porto_settings[ $post_type . '-infinite' ] ) {
						...
						foreach ( $posts->posts as $post ) {
							$post_taxs = wp_get_post_terms( (int) $post->ID, $post_type . '_cat', array( 'fields' => 'slugs' ) );
				...
				$post_layout = isset( $_REQUEST['post_layout'] ) ? sanitize_file_name( $_REQUEST['post_layout'] ) : null;

				$template_args = array();
				if ( ! empty( $_REQUEST['image_size'] ) ) {
					$template_args['image_size'] = sanitize_text_field( $_REQUEST['image_size'] );
				}
				...
				while ( $posts->have_posts() ) {
					...
					if ( 'post' == $post_type ) {
						...
						porto_get_template_part( 'content-archive-' . sanitize_file_name( $post_type ), $post_layout, $template_args );
					}
				}

				echo '</div>';

				porto_pagination( null, false, $posts, ! empty( $_REQUEST['current_link'] ) ? esc_url( $_REQUEST['current_link'] ) : false, ! empty( $_REQUEST['page'] ) ? (int) $_REQUEST['page'] : false );
