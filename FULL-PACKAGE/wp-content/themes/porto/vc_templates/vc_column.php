...
if ( ! empty( $content_collapse ) ) {
	$el_cls .= ' content-collapse-wrap';
	if ( class_exists( 'WPBMap' ) ) {
		$sc = WPBMap::getShortCode( 'vc_column' );
		if ( ! empty( $sc['params'] ) && class_exists( 'PortoShortcodesClass' ) && method_exists( 'PortoShortcodesClass', 'get_global_hashcode' ) ) {
			foreach ( $original_atts as $key => $item ) {
				if ( in_array( $key, array( 'content_collapse_height', 'content_collapse_color' ) ) ) {
					$original_atts[ $key ] = str_replace( '"', '``', $item );
				}
			}
			$el_cls .= ' wpb_custom_' . PortoShortcodesClass::get_global_hashcode( $original_atts, 'vc_column', $sc['params'] );
			$internal_css = PortoShortcodesClass::generate_wpb_css( 'vc_column', $original_atts );
		}
	}
}
