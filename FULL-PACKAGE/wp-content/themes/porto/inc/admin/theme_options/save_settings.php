...
if ( is_admin() ) {
	...
	function porto_import_theme_settings( $plugin_options, $imported_options = '' ) {
		...
		if ( is_a( $plugin_options, 'reduxCorePanel' ) ) {
			return;
		}
		...
		// Import Appearance/Sidebar
		if ( isset( $imported_options['sidebar_options'] ) && is_array( $imported_options['sidebar_options'] ) ) {
			update_option( 'sbg_sidebars', $imported_options['sidebar_options'] );
		}
