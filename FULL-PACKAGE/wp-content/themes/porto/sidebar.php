...
$content_bottom_tracking       = 'meta-content_bottom';
$content_inner_bottom_tracking = 'meta-content_inner_bottom';
if ( is_array( $content_bottom ) ) {
	$content_bottom_tracking = $content_bottom[1];
	$content_bottom = $content_bottom[0];
}
if ( is_array( $content_inner_bottom ) ) {
	$content_inner_bottom_tracking = $content_inner_bottom[1];
	$content_inner_bottom = $content_inner_bottom[0];
}
?>
...
if ( $content_inner_bottom ) :
	...
	<div id="content-inner-bottom"><!-- begin content inner bottom -->
		...
		foreach ( explode( ',', $content_inner_bottom ) as $block ) {
			echo do_shortcode( '[porto_block name="' . $block . '" tracking="' . $content_inner_bottom_tracking . '"]' );
		}
		?>
...
$is_shop_builder = false;
if ( class_exists( 'PortoBuilders' ) && is_singular( PortoBuilders::BUILDER_SLUG ) ) {
	if ( 'shop' == get_post_meta( get_the_ID(), PortoBuilders::BUILDER_TAXONOMY_SLUG, true ) ) {
		$is_shop_builder = true;
	}
}
if ( empty( $porto_sidebar ) ) {
	$porto_sidebar = 'alternative-default';
}
...
if ( in_array( $porto_layout, porto_options_sidebars() ) ) :
	...
	<div class="col-lg-3 sidebar <?php echo 'porto-' . $porto_sidebar; ?> <?php echo str_replace( 'both-', 'left-', str_replace( 'wide-', '', $porto_layout ) ); ?><?php echo ! $mobile_sidebar ? '' : ' mobile-sidebar'; ?>"><!-- main sidebar -->
		...
		<div class="sidebar-content<?php echo ! $skeleton_lazyload ? '' : ' skeleton-loading'; ?>">
			...
			<?php endif; ?>
			<?php
			if ( 'alternative-default' != $porto_sidebar ) {
			...
			if ( $is_shop_builder ) :
				?>
			<a href="<?php echo esc_url( admin_url( 'widgets.php' ) ); ?>" class="porto-shop-template-sidebar porto-setting-link" target="_blank" title="<?php esc_attr_e( 'Sidebar Widget', 'porto' ); ?>">
...
<?php if ( in_array( $porto_layout, porto_options_both_sidebars() ) ) : ?>
	<div class="<?php echo porto_filter_output( $mobile_sidebar ? 'col-md-4 col-lg-3' : 'col-lg-3' ); ?> sidebar <?php echo 'porto-' . esc_attr( $porto_sidebar2 ); ?> right-sidebar"><!-- second sidebar -->
		<?php if ( $sticky_sidebar ) : ?>
		...
		<div class="sidebar-content">
			...
			if ( $is_shop_builder ) :
				?>
			<a href="<?php echo esc_url( admin_url( 'widgets.php' ) ); ?>#sidebar-<?php echo porto_filter_output( $porto_sidebar2 ); ?>" class="porto-shop-template-sidebar porto-setting-link" target="_blank" title="<?php esc_attr_e( 'Sidebar Widget', 'porto' ); ?>">
				<i class="porto-icon-cog"></i>
			</a>
			<?php endif; ?>
		</div>
...
if ( $content_bottom ) :
	...
	<div id="content-bottom"><!-- begin content bottom -->
		...
		foreach ( explode( ',', $content_bottom ) as $block ) {
			echo do_shortcode( '[porto_block name="' . $block . '" tracking="' . $content_bottom_tracking . '"]' );
		}
		?>
