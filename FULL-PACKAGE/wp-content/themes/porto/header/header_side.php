...
?>
<header id="header" class="header-side sticky-menu-header<?php echo empty( $porto_settings['logo-overlay'] ) || empty( $porto_settings['logo-overlay']['url'] ) ? '' : ' logo-overlay-header'; ?>" data-plugin-sticky data-plugin-options="<?php echo esc_attr( '{"autoInit": true, "minWidth": 992, "containerSelector": ".page-wrapper","autoFit":true, "paddingOffsetBottom": 0, "paddingOffsetTop": 0}' ); ?>">
		...
		<div class="side-top">
			<div class="container">
				...
				<div class="header-minicart">
					<?php
					// my account
					if ( isset( $porto_settings['header-woo-icon'] ) && is_array( $porto_settings['header-woo-icon'] ) && class_exists( 'WooCommerce' ) && ( 'simple' == porto_get_minicart_type() || 'none' == porto_get_minicart_type() ) ) {
						$el_class = '';
