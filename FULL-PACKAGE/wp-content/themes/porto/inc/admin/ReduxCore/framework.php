	...
	class ReduxFramework {
		...
		public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
			...
			if ( ! empty( $this->args['opt_name'] ) ) {
				...
				if ( empty( $this->args['page_slug'] ) ) {
					...
					} elseif ( ! empty( $this->args['page_title'] ) ) {
						$this->args['page_slug'] = sanitize_html_class( $this->args['page_title'] );
					} elseif ( ! empty( $this->args['menu_title'] ) ) {
		...
		public function get_wordpress_data( $type = false, $args = array() ) {
			...
			if ( empty( $data ) && ! empty( $type ) ) {
				...
				if ( ! empty( $type ) && empty( $data ) ) {
					...
					if ( $type == 'categories' || $type == 'category' ) {
					...
					} elseif ( $type == 'taxonomy' || $type == 'taxonomies' ) {
						...
						// if
					} elseif ( $type == 'post_type' || $type == 'post_types' ) {
					...
					} elseif ( $type == 'tags' || $type == 'tag' ) { // NOT WORKING!
						$tags = get_tags( $args );
						if ( ! empty( $tags ) ) {
							foreach ( $tags as $tag ) {
						...
						// if
					} elseif ( $type == 'menu_location' || $type == 'menu_locations' ) {
						global $_wp_registered_nav_menus;

						foreach ( $_wp_registered_nav_menus as $k => $v ) {
							$data[ $k ] = $v;
						}
					} elseif ( $type == 'image_size' || $type == 'image_sizes' ) {
					...
					} elseif ( $type == 'elusive-icons' || $type == 'elusive-icon' || $type == 'elusive' ||
					...
					} elseif ( $type == 'sidebars' || $type == 'sidebar' ) {
		...
		private function _default_cleanup() {
			...
			if ( class_exists( 'Airplane_Mode_Core' ) ) {
				...
				} elseif ( $airplane->check_status() == 'on' ) {
					$this->args['use_cdn'] = false;
				}
			}
		}
		...
		public function _options_page() {
			...
			if ( $this->args['menu_type'] == 'hidden' ) {
				...
				// No menu to add!
			} elseif ( $this->args['menu_type'] == 'submenu' ) {
				$this->add_submenu(
		...
		public function _load_page() {
			...
			if ( true == $this->show_hints ) {
				...
				// Construct message
				$msg = sprintf( __( 'Hints are tooltips that popup when %1$d the hint icon, offering addition information about the field in which they appear.  They can be %2$d d by using the link below.', 'redux-framework' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;
		...
		public function _validate_options( $plugin_options ) {
			...
			if ( $plugin_options == $this->options && ! ( isset( $_POST['_wpnonce'] ) && $_POST['_wpnonce'] ) ) {
				return $plugin_options;
			}
			...
			if ( ( isset( $plugin_options['import_code'] ) && ! empty( $plugin_options['import_code'] ) ) || ( isset( $plugin_options['import_link'] ) && ! empty( $plugin_options['import_link'] ) ) ) {
				...
				if ( ! empty( $imported_options ) && is_array( $imported_options ) && isset( $imported_options['redux-backup'] ) && $imported_options['redux-backup'] == '1' ) {
					 ...
					 */
					do_action_ref_array(
						"redux/options/{$this->args['opt_name']}/import",
						array(
							&$plugin_options,
							$imported_options,
			 ...
			 */
			do_action_ref_array(
				"redux-validate-{$this->args['opt_name']}",
				array(
					&$plugin_options,
					$this->options,
				)
			 ...
			 */
			do_action_ref_array(
				"redux/options/{$this->args['opt_name']}/validate",
				array(
					&$plugin_options,
					$this->options,
					$this->transients['changed_values'],
		...
		public function ajax_save() {
			...
			if ( ! empty( $_POST['data'] ) && ! empty( $redux->args['opt_name'] ) ) {
				...
				$values = $this->redux_parse_str( $post_data );
				$values = $values[ $redux->args['opt_name'] ];

				// For Mobile Menu Tab
				if ( ! array_key_exists('show-mobile-menus', $values) ) {
					$values[ 'show-mobile-menus' ] = '';
				}
				if ( ! empty( $values ) ) {
					...
					try {
						...
						$validated_values = $redux->_validate_options( $values );
						if ( ! empty( $validated_values ) ) {
							foreach ( $validated_values as $key => $value ) {
								$redux->set( $key, $value );
							}
						}
		...
		public function _field_input( $field, $v = null ) {
			...
			if ( isset( $field['callback'] ) && ( is_callable( $field['callback'] ) || ( is_string( $field['callback'] ) && function_exists( $field['callback'] ) ) ) ) {
				 ...
				 */
				do_action_ref_array(
					"redux/field/{$this->args['opt_name']}/{$field['type']}/callback/before",
					array(
						&$field,
						&$value,
					)
				 ...
				 */
				do_action_ref_array(
					"redux/field/{$this->args['opt_name']}/callback/before",
					array(
						&$field,
						&$value,
					)
				 ...
				 */
				do_action_ref_array(
					"redux/field/{$this->args['opt_name']}/{$field['type']}/callback/after",
					array(
						&$field,
						&$value,
					)
				 ...
				 */
				do_action_ref_array(
					"redux/field/{$this->args['opt_name']}/callback/after",
					array(
						&$field,
						&$value,
					)
			...
			if ( isset( $field['type'] ) ) {
				...
				if ( class_exists( $field_class ) ) {
					 ...
					 */
					do_action_ref_array(
						"redux/field/{$this->args['opt_name']}/{$field['type']}/render/before",
						array(
							&$field,
							&$value,
						)
					 ...
					 */
					do_action_ref_array(
						"redux/field/{$this->args['opt_name']}/render/before",
						array(
							&$field,
							&$value,
						)
					...
					*/
					 ...
					 */
					do_action_ref_array(
						"redux/field/{$this->args['opt_name']}/{$field['type']}/fieldset/before/{$this->args['opt_name']}",
						array(
							&$field,
							&$value,
						)
					 ...
					 */
					do_action_ref_array(
						"redux/field/{$this->args['opt_name']}/fieldset/before/{$this->args['opt_name']}",
						array(
							&$field,
							&$value,
						)
					 ...
					 */
					do_action_ref_array(
						"redux/field/{$this->args['opt_name']}/{$field['type']}/fieldset/after/{$this->args['opt_name']}",
						array(
							&$field,
							&$value,
						)
					 ...
					 */
					do_action_ref_array(
						"redux/field/{$this->args['opt_name']}/fieldset/after/{$this->args['opt_name']}",
						array(
							&$field,
							&$value,
						)
