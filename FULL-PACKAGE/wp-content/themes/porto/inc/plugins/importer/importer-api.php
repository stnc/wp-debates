...
class Porto_Importer_API {
	...
	protected $url = array(
		...
		'patcher_server'     => PORTO_API_URL . 'download/patcher.php',
	);
	...
	public function generate_args( $ish = true ) {
		...
		if ( $ish && function_exists( 'Porto' ) && Porto()->is_envato_hosted() ) {
	...
	public function get_patch_files() {
		if ( defined( 'PORTO_FUNC_VERSION' ) ) {
			...
			$args = $this->generate_args( false );
			$url = $this->get_url( 'patcher_server' );
			if ( isset( $args['code'] ) ) {
				$args = array(
					'theme_version' => PORTO_VERSION,
					'func_version'  => PORTO_FUNC_VERSION,
					'code'          => $args['code'],
				);
				$url = add_query_arg( $args, $url );
			}
	...
	public function get_patch_content( $patches_data ) {
		if ( defined( 'PORTO_FUNC_VERSION' ) ) {
			...
			if ( $response ) {
				$gz_response = gzdecode( $response );
				if ( $gz_response ) {
					return json_decode( $gz_response, true );
				} else {
