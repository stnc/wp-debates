...
if ( ! class_exists( 'Porto_Dynamic_Style' ) ) :
	class Porto_Dynamic_Style {
		...
		public function init_vc_custom_styles() {
			...
			if ( version_compare( WPB_VC_VERSION, '7.7', '>=' ) ) {
				remove_action( 'wp_head', array( vc_manager()->vc(), 'addShortcodesCss' ), 1000 );
				ob_start();
				vc_manager()->vc()->addShortcodesCss();
		...
		public function enqueue_style() {
			...
			if ( defined( 'WPB_VC_VERSION' ) && ! empty( $porto_settings_optimize['shortcodes_to_remove'] ) ) {
				$upload_dir = wp_upload_dir();
