    ...
    var ElevateZoom = {
            ...
            startZoom: function( ) {
                ...
                var userAgent = navigator.userAgent.toLowerCase(),
                    isChromeDesktop = (userAgent.indexOf('chrome') > -1 && ((userAgent.indexOf('windows') > -1) || (userAgent.indexOf('macintosh') > -1) || (userAgent.indexOf('linux') > -1)) && userAgent.indexOf('mobile') < 0 && userAgent.indexOf('android') < 0),
                    touch_capable = ('ontouchstart' in window && !isChromeDesktop);

                //touch events
                if ('dbltouch' == self.options.zoomActivation && window.theme && theme.is_device_mobile && touch_capable) {
                    if ( ! $.fn.doubletap ) {
                        $.fn.doubletap = function( fn ) {
                            this.on( 'doubletap', fn );
                        };
                        // doubletap Event:
                        $.event.special.doubletap = {
                            setup: function () {
                                ...
                                $this.on('touchstart', function doubleTapFunc1(e) {
                                    ...
                                    return true;
                                }).on('touchend', function doubleTapFunc2(e) {
                                    var now = Date.now();
                                    ...
                                    if (delta < 350 && (e.target == origTarget) && delta > 100) {
                            ...
                            remove: function () {
                                $(this).off('touchstart', $(this).data.callee1).off('touchend', $(this).data.callee2);
                            }
                        };
                    }

                    self.zoomContainer.on('doubletap', function(e) {
                        var toShow = false;
                        if ('inner' == self.options.zoomType) {
                    ...
                    self.zoomContainer.on('touchmove', function(e){ 
                        if ( ('inner' == self.options.zoomType && !self.isWindowActive ) || ( self.options.showLens && !self.isLensActive ) ) {
                            return;
                        }
                        e.preventDefault();
                        e.stopPropagation();
