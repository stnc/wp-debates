...
function porto_grid_list_toggle( $el_class = false, $icon_grid = '', $icon_list = '' ) {
	...
	<div class="gridlist-toggle<?php echo ! $el_class ? '' : ' ' . esc_attr( $el_class ); ?>">
		<a href="#" id="grid" class="<?php echo esc_attr( $icon_grid ), 'grid' == $view_mode ? ' active' : ''; ?>" title="<?php esc_attr_e( 'Grid View', 'porto' ); ?>"></a><a href="#" id="list" class="<?php echo esc_attr( $icon_list ), 'list' == $view_mode ? ' active' : ''; ?>" title="<?php esc_attr_e( 'List View', 'porto' ); ?>"></a>
	</div>
	<?php
}
...
function porto_loop_product_thumbnail() {
	...
	if ( ! empty( $gallery ) && ! empty( $porto_settings['category-image-hover'] ) ) {
		...
		if ( $show_hover_img ) {
			...
			$attachment_image = apply_filters( 'porto_lazy_load_images', wp_get_attachment_image( $first_image_id, $size, false, array( 'class' => 'hover-image' ) ) );
		}
	}
		...
		if ( $product ) {
			...
			foreach ( $attribute_terms as $term ) {
				...
				$html     .= '<li' . ( sanitize_title( $args['selected'] ) === $term['slug'] ? ' class="active"' : '' ) . '>';
					$html .= '<a href="#" class="' . $a_class . '" data-value="' . esc_attr( $term['slug'] ) . '" ' . ( $is_active ? ' class="active"' : '' ) . $a_attrs . '>' . esc_html( apply_filters( 'woocommerce_variation_option_name', 'label' == $attr_type && $label_value ? $label_value : $term['label'], isset( $term['term_id'] ) ? $term : null, $attribute, $product ) ) . '</a>';
				$html     .= '</li>';
			}
			$select_html .= '</select>';
		}
	...
	if ( ( ( ! isset( $porto_woocommerce_loop['widget'] ) || ! $porto_woocommerce_loop['widget'] ) && $product && $product->is_on_sale() ) || ( $dynamic_product && $dynamic_product->is_on_sale() ) ) {
		...
		if ( $product->is_type( 'variable' ) ) {
			...
			foreach ( $variations as $variation ) {
				$from_date = get_post_meta( $variation['variation_id'], '_sale_price_dates_from', true );
				if ( ! $from_date || (int) $from_date > $cur_time ) {
					$date_diff = false;
					continue;
				}
...
function porto_woocommerce_display_variation_on_shop_page( $el_class = '' ) {
...
if ( ! function_exists( 'porto_template_loop_compare' ) ) {
	function porto_template_loop_compare( $extra_class = '', $hide_title = false, $icon_cls = '', $icon_pos = 'left', $icon_cls_added = '' ) {
		...
		if ( defined( 'YITH_WOOCOMPARE' ) && isset( $product ) ) {
			...
			printf( '<a href="%s" class="%s" title="%s" data-product_id="%d"' . ( $icon_cls_added ? ' data-added_icon="' . esc_attr( $icon_cls_added ) . '"' . ( 'right' == $icon_pos ? ' data-icon_pos="right"' : '' ) : '' ) . ' data-hide_title="' . ( $hide_title ? 'hide' : 'show' ) . '">%s</a>', esc_url( $url ), esc_attr( $css_class ), esc_attr( $title_text ), $product_id, $button_text );
		}
	}
}
