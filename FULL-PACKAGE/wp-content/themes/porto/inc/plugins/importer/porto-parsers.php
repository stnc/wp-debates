 ...
 */
if ( ! class_exists( 'PORTO_WXR_Parser' ) ) {
	class PORTO_WXR_Parser {
		function parse( $file ) {
			...
			if ( extension_loaded( 'simplexml' ) && class_exists( 'DOMDocument' ) ) {
				...
				$parser = new PORTO_WXR_Parser_XML;
				$result = $parser->parse( $file );
			...
			if ( isset( $result ) && defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
				...
				echo '</pre>';
				echo '<p><strong>' . esc_html__( 'There was an error when reading this WXR file', 'porto' ) . '</strong><br />';
				esc_html_e( 'Details are shown above. The importer will now try again with a different parser...', 'porto' ) . '</p>';
			}

			// use regular expressions if nothing else available or this is bad XML
			$parser = new PORTO_WXR_Parser_Regex;
 ...
 */
if ( ! class_exists( 'PORTO_WXR_Parser_SimpleXML' ) ) {
	class PORTO_WXR_Parser_SimpleXML {
		function parse( $file ) {
			...
			$success = $dom->loadXML( $file_contents );
			if ( ! is_null( $old_value ) ) {
				libxml_disable_entity_loader( $old_value );
			}

			if ( ! $success || isset( $dom->doctype ) ) {
				return new WP_Error( 'SimpleXML_parse_error', __( 'There was an error when reading this WXR file', 'porto' ), libxml_get_errors() );
			}
			...
			if ( ! $wxr_version ) {
				return new WP_Error( 'PORTO_WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'porto' ) );
			}
 ...
 */
if ( ! class_exists( 'PORTO_WXR_Parser_XML' ) ) {
	class PORTO_WXR_Parser_XML {
 ...
 */
if ( ! class_exists( 'PORTO_WXR_Parser_Regex' ) ) {
	class PORTO_WXR_Parser_Regex {
