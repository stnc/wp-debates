    ...
    $.fn.portoInfiniteScroll = function( page_path ) {
        return this.map( function() {
            ...
            if ( $posts_wrap.closest( '.archive-posts' ).length ) {
                var $paginationSibling = $posts_wrap.siblings( '.pagination-wrap' );
                if ( $paginationSibling.length == 0 && $posts_wrap.parent( '.yit-wcan-container' ).length ) {
                    $paginationSibling = $posts_wrap.parent( '.yit-wcan-container' ).siblings( '.pagination-wrap' );
                }
                var next_page_url = $paginationSibling.find( '.next' ).attr( 'href' );
                if ( !next_page_url ) {
            ...
            if ( ! $( item_selector ).length ) {
                return;
            }

            var selector = $this.find( '.porto-ajax-filter li.active' ).attr( 'data-filter' );
            var current_cat = '*' == selector ? '' : selector;
            var taxonomy = '';
            if ( ( typeof current_cat == 'undefined' || current_cat == '' ) && $wrap.find( 'input[type=hidden].category' ).length ) {
                current_cat = $wrap.find( 'input[type=hidden].category' ).val();
            }
            if ( $wrap.find( 'input[type=hidden].taxonomy' ).length ) {
                taxonomy = $wrap.find( 'input[type=hidden].taxonomy' ).val();
                ajax_call_url = ajax_call_url.replace( /&category=[^&]*&/, '&category=' + current_cat + '&taxonomy=' + taxonomy + '&' );
            } else if ( typeof current_cat != 'undefined' ) {
                ajax_call_url = ajax_call_url.replace( /&category=[^&]*&/, '&category=' + current_cat + '&' );
            }
