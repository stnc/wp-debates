...
global $porto_settings, $porto_product_layout, $porto_layout, $porto_shop_filter_layout;
...
<?php if ( $is_wide ) : ?>
	@media (min-width: 1500px) {
		...
		<?php if ( ( 'wide-left-sidebar' != $porto_layout && 'wide-right-sidebar' != $porto_layout ) || ( ! isset( $porto_shop_filter_layout ) || 'offcanvas' != $porto_shop_filter_layout ) ) : ?>
...
if ( isset( $porto_product_layout ) && $porto_product_layout ) :
	...
	if ( porto_is_product() ) :
		...
		?>
			...
			.sticky-product.sticky-ready { transition: <?php echo porto_filter_output( $left ); ?> .3s, visibility 0.3s, opacity 0.3s, transform 0.3s, top 0.3s ease; }
...
if ( $body_bg_color || $body_bg_image || $body_bg_repeat || $body_bg_size || $body_bg_attachment || $body_bg_position
	...
	body {
		...
		elseif ( ! empty( $body_bg_image ) && is_string( $body_bg_image ) ) :
			?>
...
if ( $porto_shop_filter_layout ) :
	...
	if ( 'horizontal' === $porto_shop_filter_layout ) {
		?>
		<?php if ( $is_wide && in_array( $porto_layout, porto_options_sidebars() ) ) : ?>
		@media (min-width: 1500px) {
