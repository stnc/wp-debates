...
if ( ! class_exists( 'ReduxFramework_extension_customizer' ) ) {
	...
	class ReduxFramework_extension_customizer {
		...
		public function __construct( $parent ) {
			...
			if ( isset( $_POST['wp_customize'] ) && $_POST['wp_customize'] == 'on' && isset( $_POST['customized'] ) && ! empty( $_POST['customized'] ) && ( ! isset( $_POST['action'] ) || empty( $_POST['action'] ) ) ) {
				add_action(
		...
		public function render( $control ) {
			...
			if ( isset( $this->parent->field_head[ $field['id'] ] ) ) {
		...
		public function _register_customizer_controls( $wp_customize ) {
			...
			foreach ( $this->parent->sections as $key => $section ) {
				...
				foreach ( $section['fields'] as $skey => $option ) {
					...
					if ( $option['type'] == 'info' && $option['id'] === '1' ) {
						$info_index++;
						$option['id'] = $section['id'] . '_info' . $info_index;
					}
					...
					if ( $option['type'] != 'heading' && $option['type'] != 'info' && $option['type'] != 'import_export' && ! empty( $option['type'] ) ) {

						if ( ( ( isset( $option['transport'] ) && 'postMessage' == $option['transport'] ) || ( isset( $section['transport'] ) && 'postMessage' == $section['transport'] && ( ! isset( $option['transport'] ) || 'refresh' != $option['transport'] ) ) ) && $wp_customize->selective_refresh ) {
					...
					if ( $option['id'] == $this->parent->args['opt_name'] . '[css-code]' ) {
					...
					} elseif ( $option['id'] == $this->parent->args['opt_name'] . '[js-code-head]' || $option['id'] == $this->parent->args['opt_name'] . '[js-code]' ) {
						$wp_customize->add_control(
							new WP_Customize_Code_Editor_Control(
								$wp_customize,
								$option['id'],
								array(
									'label'     => html_entity_decode( $option['title'] ),
									'section'   => $section['id'],
									'settings'  => $option['id'],
						...
						$option_settings = ( $option['type'] != 'info' ? $option['id'] : array() );
						$wp_customize->add_control(
							new $class_name(
								$wp_customize,
								$option['id'],
