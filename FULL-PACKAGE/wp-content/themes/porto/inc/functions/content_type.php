		...
		if ( is_singular() ) {
			...
			if ( ! is_home() && is_front_page() ) {
				...
				if ( ( ! $blog_id || ! $value ) && isset( $porto_settings[ 'blog-' . $meta_key ] ) ) {
					...
					if ( in_array( 'blog-' . $meta_key, $content_pos ) ) {
						$value = array( $value, 'option-blog-' . $meta_key );
					}
				}
			} elseif ( is_home() || is_front_page() ) {
				if ( isset( $porto_settings[ $meta_key ] ) ) {
...
function porto_meta_layout() {
	...
	$shop_builder = false;
	if ( class_exists( 'PortoBuilders' ) && is_singular( PortoBuilders::BUILDER_SLUG ) && 'shop' == get_post_meta( get_the_ID(), PortoBuilders::BUILDER_TAXONOMY_SLUG, true ) ) {
		$shop_builder = true;
	}
	...
	} elseif ( is_archive() || is_search() || $shop_builder ) {
		if ( ( function_exists( 'porto_is_shop' ) && porto_is_shop() && ! is_product_category() ) || $shop_builder ) {
			if ( $default ) {
