...
if ( ! class_exists( 'Porto_Performance' ) ) :
	class Porto_Performance {
		...
		public function __construct() {
			...
			if ( porto_is_ajax() && isset( $_REQUEST['load_posts_only'] ) ) {
				return;
			}
			...
			foreach ( $this->removed_resources as $index => $file ) {
				if ( $rc_type == $file['type'] ) {
					...
					if ( 'css' == $rc_type ) {
						...
						if ( 'vc_material-css' == $index ) {
							$contents = str_replace( 'url(fonts', 'url(' . plugins_url( '/', WPB_PLUGIN_FILE ) . 'assets/css/lib/vc-material/fonts', $contents );
						}
		...
		public function remove_resources( $rc_type = 'css' ) {
			...
			foreach ( $wp_resources->to_do as $enqueued_index => $file ) {
				...
				if ( 0 === strpos( $file, 'vc_google_fonts_' ) ) {
					continue;
				}
