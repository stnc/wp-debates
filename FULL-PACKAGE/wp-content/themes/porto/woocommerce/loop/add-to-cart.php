...
<div class="add-links-wrap">
		...
		if ( ! $catalog_mode ) {
			if ( version_compare( $porto_woo_version, '3.3', '<' ) ) {
				...
				echo apply_filters(
					'woocommerce_loop_add_to_cart_link', // WPCS: XSS ok.
					sprintf(
						'<%s href="%s" aria-describedby="woocommerce_loop_add_to_cart_link_describedby_%s" data-quantity="%s" class="' . $viewcart_style . ' %s" %s>%s</%s>',
						( $product->is_purchasable() && $product->is_in_stock() && isset( $porto_settings['category-addlinks-convert'] ) && $porto_settings['category-addlinks-convert'] ) ? 'span' : 'a',
			...
			$more_link   = $product->get_permalink();
			$more_target = '';
			if ( 'all' === $porto_settings['catalog-readmore-archive'] ) {
			...
			echo apply_filters(
				'woocommerce_loop_add_to_cart_link', // WPCS: XSS ok.
				sprintf(
					'<a href="%s" aria-describedby="woocommerce_loop_add_to_cart_link_describedby_%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" data-quantity="%s" class="button ' . $viewcart_style . ' %s product_type_%s" %s %s>%s</a>',
					esc_url( $more_link ),
					esc_attr( $product->get_id() ),
					esc_attr( $product->get_id() ),
					esc_attr( $product->get_sku() ),
	...
	<?php
		...
		// Hide for Fibosearch
		if ( ! wp_doing_ajax() || ( empty( $_REQUEST['action'] ) || 'dgwt_wcas_result_details' != $_REQUEST['action'] ) ) {
			if ( $wishlist ) {
