...
require PORTO_PLUGINS . '/importer/porto-parsers.php';
 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
	class Porto_WP_Import extends WP_Importer {
		...
		function import( $file, $process = 'import_start', $index = 0 ) {
			...
			if ( 'import_start' == $process ) {
				delete_option( 'porto_import_processed_authors' );
				delete_option( 'porto_import_author_mapping' );
				delete_option( 'porto_import_processed_terms' );
				delete_option( 'porto_import_processed_posts' );
				delete_option( 'porto_import_post_orphans' );
				delete_option( 'porto_import_processed_menu_items' );
				delete_option( 'porto_import_menu_item_orphans' );
				delete_option( 'porto_import_missing_menu_items' );
				delete_option( 'porto_import_url_remap' );
				delete_option( 'porto_import_featured_images' );
			}

			$this->processed_authors    = get_option( 'porto_import_processed_authors', array() );
			$this->author_mapping       = get_option( 'porto_import_author_mapping', array() );
			$this->processed_terms      = get_option( 'porto_import_processed_terms', array() );
			$this->processed_posts      = get_option( 'porto_import_processed_posts', array() );
			$this->post_orphans         = get_option( 'porto_import_post_orphans', array() );
			$this->processed_menu_items = get_option( 'porto_import_processed_menu_items', array() );
			$this->menu_item_orphans    = get_option( 'porto_import_menu_item_orphans', array() );
			$this->missing_menu_items   = get_option( 'porto_import_missing_menu_items', array() );
			$this->url_remap            = get_option( 'porto_import_url_remap', array() );
			$this->featured_images      = get_option( 'porto_import_featured_images', array() );
			...
			switch ( $process ) {
				case 'import_start':
					...
					if ( ! in_array( $demo, $extra_demos ) && function_exists( 'wc_get_attribute_taxonomies' ) ) {
						$attribute_names  = array( 'Color', 'Size', 'Brand', 'Make', 'Model', 'Product Year' );
						if ( 'shop48' == $demo || 'elementor-shop48' ) {
							$attribute_names = array_merge(
								$attribute_names,
								array(
									'Center Stone',
									'Diamond Clarity',
						...
						foreach ( $attribute_names as $attribute_name ) {
							...
							if ( ! in_array( $attribute_name, $registered_attrs ) ) {
								...
								} elseif ( 'Make' === $attribute_name || 'Model' === $attribute_name || 'Product Year' === $attribute_name ) {
									$attribute['attribute_type'] = 'label';
								} elseif ( 'Metal' === $attribute_name ) {
									$attribute['attribute_type'] = 'image';
								}
								if ( 'Product Year' === $attribute_name ) {
									$attribute['attribute_name'] = 'product-year';
								} elseif ( 'Center Stone' === $attribute_name ) {
									$attribute['attribute_name'] = 'center-stone';
								} elseif ( 'Diamond Clarity' === $attribute_name ) {
									$attribute['attribute_name'] = 'diamond-clarity';
								} elseif ( 'Diamond Color' === $attribute_name ) {
									$attribute['attribute_name'] = 'diamond-color';
								} elseif ( 'Metal Color' === $attribute_name ) {
									$attribute['attribute_name'] = 'metal-color';
								} elseif ( 'Metal Finish' === $attribute_name ) {
									$attribute['attribute_name'] = 'metal-finish';
								} elseif ( 'Metal Type' === $attribute_name ) {
									$attribute['attribute_name'] = 'metal-type';
								} elseif ( 'Stone Shape' === $attribute_name ) {
									$attribute['attribute_name'] = 'stone-shape';
								} elseif ( 'Total Diamond Weight' === $attribute_name ) {
									$attribute['attribute_name'] = 'total-diamond-weight';
								}
					...
					dbDelta( $sql );
					$this->process_categories();
					$response['message'] = __( 'Imported categories', 'porto' );
					$response['process'] = 'process_tags';
					break;
				case 'process_tags':
					$this->process_tags();
					$response['message'] = __( 'Imported tags', 'porto' );
					$response['process'] = 'process_terms';
					break;
				case 'process_terms':
					$this->process_terms();
					$response['message'] = __( 'Imported terms', 'porto' );
					$response['process'] = 'process_posts';
					break;
				case 'process_posts':
					if ( $index >= count( $this->posts ) ) {
						$this->backfill_parents();
						$response['message'] = __( 'Imported posts', 'porto' );
				...
				case 'process_menus':
					if ( $index >= count( $this->missing_menu_items ) ) {
						...
						$response['message'] = __( 'Importing menus', 'porto' );
						$response['process'] = 'process_menus_orphans';
						$response['index']   = ++$index;
					} else {
						$response['count'] = count( $this->missing_menu_items );
						$this->process_menus( $index );
						$response['message'] = __( 'Processing menus', 'porto' );
						$response['process'] = 'process_menus';
						$response['item'] = $this->missing_menu_items[$index];
						$response['index']   = ++$index;
					}
					break;
				...
				case 'remap_featured_images':
					...
					$response['message'] = __( 'Successfully imported dummy content!', 'porto' );
					$response['process'] = 'complete';
					break;
			}

			update_option( 'porto_import_processed_authors', $this->processed_authors );
			update_option( 'porto_import_author_mapping', $this->author_mapping );
			update_option( 'porto_import_processed_terms', $this->processed_terms );
			update_option( 'porto_import_processed_posts', $this->processed_posts );
			update_option( 'porto_import_post_orphans', $this->post_orphans );
			update_option( 'porto_import_processed_menu_items', $this->processed_menu_items );
			update_option( 'porto_import_menu_item_orphans', $this->menu_item_orphans );
			update_option( 'porto_import_missing_menu_items', $this->missing_menu_items );
			update_option( 'porto_import_url_remap', $this->url_remap );
			update_option( 'porto_import_featured_images', $this->featured_images );
		...
		function handle_upload() {
			...
			if ( isset( $file['error'] ) ) {
				...
				printf( esc_html__( 'The export file could not be found at %s. It is likely that this was caused by a permissions problem.', 'porto' ), '<code>' . esc_html( $file['file'] ) . '</code>' );
				echo '</p>';
				return false;
			}
			...
			foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
				...
				if ( ! empty( $_POST['user_map'][ $i ] ) ) {
					...
					if ( ! is_wp_error( $user_id ) ) {
						...
						printf( __( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'porto' ), esc_html( $this->authors[ $old_login ]['author_display_name'] ) );
		...
		function process_posts( $index = 0 ) {
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				//                    printf( __('%s &#8220;%s&#8221; already exists.', 'porto'), $post_type_object->labels->singular_name, esc_html($post['post_title']) );
				//                    echo '<br />';
			...
			if ( ! empty( $post['postmeta'] ) ) {
				...
				foreach ( $post['postmeta'] as $meta ) {
					...
					if ( $key ) {
						...
						if ( 'porto_builder' == $post['post_type'] && 'porto_blocks_style_options_css' == $key && false !== strpos( $post['post_content'], 'sw-themes.com' ) ) {
							// Update Type Builder
							PortoShortcodesClass::get_instance()->save_meta_values( $post_id, (object) $post );
						} else {
		...
		function process_menu_item( $item ) {
			...
			} elseif ( 'post_type_archive' == $_menu_item_type && isset( $_menu_item_object ) ) {
		...
		function process_attachment( $post, $url ) {
			...
			do_action( 'porto_importer_insert_attachment', $post_id, isset( $post['import_id'] ) ? $post['import_id'] : false );
		...
		function process_menus_orphans( $index = 0 ) {
			...
			if ( ! isset( $menu_item_orphans_values[ $index ] ) ) {
				return;
			}
...
} // class_exists( 'PORTO_WP_Importer' )
