...
(function( $ ) {
    ...
    $.redux.initButtonsetQtip = function( selector ) {
        if ( $().qtip ) {
            // Shadow
            var shadow = '';
            ...
            $( selector ).find( 'div.redux-hint-qtip' ).each(
                function() {
                    if ( $( this ).hasClass( 'qtip-added' ) ) {
                        return;
                    }
                    $(this).addClass( 'qtip-added' );
                    $( this ).qtip(
                        {
                            content: {
    ...
    $.redux.get_container_value = function( id ) {
        ...
        if ( redux.customizer && value !== null && typeof value === 'object' && value.hasOwnProperty( id ) ) {
            value = value[id];
        }
    ...
    $.redux.check_dependencies = function( variable ) {
        ...
        $.each(
            redux.required[id], function( child, dependents ) {
                ...
                var current = $( this ),
                    show = false,
                    childFieldset = $( 'fieldset#' + redux.args.opt_name + '-' + child ),
                    tr = childFieldset.parents( 'tr:first' ).length ? childFieldset.parents( 'tr:first' ) : childFieldset.parents( '.customize-control:first' );
            ...
            $( '#redux-footer' ).css(
                {
                    ...
                    bottom: '0',
                    width: stickyWidth,
                    right: $( '#redux-footer' ).closest('.wrap').css('margin-right')
                }
            );
